...
if (!CEREBRAS_API_KEY) {
    console.error('❌ Error: Missing CEREBRAS_API_KEY in .env file.');
    console.error('Get your API key from Cerebras Cloud.');
    process.exit(1);
}
...
async function translateText(text, fromLang = 'it', toLang = 'en') {
    const response = await fetch('https://api.cerebras.ai/v1/chat/completions', {
        ...
        body: JSON.stringify({
            model: 'gpt-oss-120b', // Using the requested model
            messages: [
                {
                    role: 'system',
...
async function runTranslations() {
    console.log('🌍 Starting Translation Service (Cerebras)\n');
